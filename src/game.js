...
// Precarga de recursos fuera de la clase Game
const imagePaths = [
  'images/avion_enemy1.png',
  'images/helicoptero_enemy3.png',
  'images/tanque_enemy2.png',
  'images/avion.png',
  'images/logo.png'
];
...
const soundPaths = [
  'sounds/bomb.ogg',
  'sounds/click.ogg',
  'sounds/explosion_word.ogg',
  'sounds/game_over.ogg',
  'sounds/impacto_to_warrior.ogg',
  'sounds/transition.ogg',
  'sounds/type.ogg',
  'sounds/typing_error.ogg',
  'music/gaming_music2.ogg',
  'music/menu_music.ogg'
];

function preloadWithProgress(imagePaths, soundPaths) {
  const total = imagePaths.length + soundPaths.length;
  let loaded = 0;
  ...
  const preloadImages = imagePaths.map(src => new Promise(resolve => {
    const img = new Image();
    img.src = src;
  ...
  const preloadSounds = soundPaths.map(src => new Promise(resolve => {
...
export class Game {
  constructor() {
    ...
    this.tildesObligatorias = true // Controla si las tildes son obligatorias
  ...
  async init() {
    ...
    // Precarga recursos con barra de progreso
    await preloadWithProgress(imagePaths, soundPaths);

    // Oculta el loader y continúa con la inicialización
  ...
  setupEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (
        ...
        if (lifeBonusDiv) {
          ...
          setTimeout(() => {
            lifeBonusDiv.classList.add('fade-out');
          }, 2500); // Empieza a desvanecer después de 2.5 segundos
          setTimeout(() => {
            lifeBonusDiv.classList.remove('show');
            lifeBonusDiv.classList.remove('fade-out');
            lifeBonusDiv.classList.add('hidden');
          }, 3700); // Oculta completamente después de 2.5 + 1.2 segundos
        }
        // Sonido de bomba
    ...
    // Botón para alternar tildes
    const toggleAccentsBtn = document.getElementById('toggle-accents-btn');
    if (toggleAccentsBtn) {
      toggleAccentsBtn.addEventListener('click', () => {
        this.tildesObligatorias = !this.tildesObligatorias;
        toggleAccentsBtn.textContent = this.tildesObligatorias ? 'Tildes: ON' : 'Tildes: OFF';
        toggleAccentsBtn.style.background = this.tildesObligatorias ? '#00ffff' : '#ffaa00';
        toggleAccentsBtn.style.color = this.tildesObligatorias ? '#222' : '#222';
        toggleAccentsBtn.style.boxShadow = this.tildesObligatorias ? '0 0 8px #00ffff99' : '0 0 8px #ffaa0099';
        // Recupera el focus en el input de palabras
        const wordInput = document.getElementById('word-input');
    ...
    const gameLoop = (currentTime) => {
      ...
      if (playerShip) {
        ...
        if (this.currentTargetWord) {
          ...
          const targetAngle = Math.atan2(dx, shipY - wordY) * (180 / Math.PI);
          ...
          // Si no hay palabra objetivo, solo aplica el último ángulo guardado (sin modificarlo)
          playerShip.style.transition = 'transform 0.08s cubic-bezier(.4,0,.2,1)';
          playerShip.style.transform = `translateX(-50%) rotate(${playerShip._currentAngle}deg)`;
        }
      }
  ...
  findTargetWord(input) {
    ...
    let matchFn;
    if (this.tildesObligatorias) {
      matchFn = word => word.originalText.toLowerCase().startsWith(input) && word.typedLength === 0;
    } else {
      const inputSinTilde = this.quitarTildes(input);
      matchFn = word => this.quitarTildes(word.originalText.toLowerCase()).startsWith(inputSinTilde) && word.typedLength === 0;
    }
    const matchingWord = this.words.find(matchFn);
    if (matchingWord) {
  ...
  processTyping(input) {
    ...
    let match = false;
    if (this.tildesObligatorias) {
      match = targetText.startsWith(input);
    } else {
      match = this.quitarTildes(targetText).startsWith(this.quitarTildes(input));
    }
    if (match) {
      ...
      let complete = false;
      if (this.tildesObligatorias) {
        complete = input === targetText;
      } else {
        complete = this.quitarTildes(input) === this.quitarTildes(targetText);
      }
      if (complete) {
